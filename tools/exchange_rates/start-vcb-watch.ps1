...
$pidPath = Join-Path $runtimeDir "vcb-watch.pid"
$stdoutPath = Join-Path $runtimeDir "vcb-watch.stdout.log"
$stderrPath = Join-Path $runtimeDir "vcb-watch.stderr.log"
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($rawState -match "^\d+$") {
        $existingPid = [int]$rawState
    } else {
        $existingState = $rawState | ConvertFrom-Json
        $existingPid = [int]$existingState.process_id
    }
    $existingProcess = Get-Process -Id $existingPid -ErrorAction SilentlyContinue
    if ($existingProcess -and $existingProcess.ProcessName -like "python*") {
        Write-Output "VCB collector is already running with PID $existingPid."
        exit 0
    }
...
Write-Output "Started VCB collector with PID $($process.Id)."
